...
#
# The download-and-verify shape is borrowed from herdr-palette's
# scripts/fetch-or-build.sh (itself after herdr-file-viewer's, by Saeed
# Marzban, MIT).
#
...
# Last gate before installing: run it once. Without HERDR_SOCKET_PATH the
# binary prints an error and exits 1 before any terminal setup (src/main.rs),
# so this has no side effect - what it proves is that the kernel accepts the
# binary at all. The variable is unset explicitly because this build step is
# itself launched by herdr, whose environment may carry it; with it set, the
# binary would connect to the live socket and start its TUI. 126/127 are the
# shell's "could not execute" codes, which is what a wrong triple looks like
# when uname or getconf misled the mapping above.
env -u HERDR_SOCKET_PATH -- "$tmpbin" </dev/null >/dev/null 2>&1
rc=$?
