...
setup() {
  ...
  # The real install runs inside herdr, whose environment carries this
  # variable; the script must unset it around its exec check.
  export HERDR_SOCKET_PATH="$BATS_TEST_TMPDIR/fake.sock"
...
# asset is a script that exits with EXIT_CODE (default 1, what the real binary
# does when HERDR_SOCKET_PATH is unset), which is what the script's exec
# check reads. It also fails loudly if the variable leaked through: the real
